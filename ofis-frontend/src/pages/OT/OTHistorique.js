...
const OTHistorique = () => {
  ...
  const chargerHistorique = async () => {
    setLoading(true);
    try {
      const data = await otService.getHistorique(id);
  ...
  const getIcon = (type) => {
    switch (type) {
      ...
      case 'cr': return <FaFileAlt style={{ color: '#8b5cf6' }} />;
  ...
  const getTypeLabel = (type) => {
    switch (type) {
      case 'suivi': return 'Heures travaillées';
  ...
  const filteredHistorique = historique.filter(item => {
    ...
    if (searchTerm) {
      const text = `${item.description || ''} ${item.technicien || ''} ${item.nom || ''}`.toLowerCase();
  ...
  return (
    <div className="dashboard-page">
      <div className="page-header">
        ...
        <h1>Historique de l'OT : {ot.reference}</h1>
      </div>

      <Card>
        <p><strong>Objet :</strong> {ot.objet || '-'}</p>
        <p><strong>Client :</strong> {ot.client_rapport_name || '-'}</p>
        <p><strong>Techniciens :</strong> {ot.techniciens_names?.join(', ') || '-'}</p>
        ...
        <div style={{ marginBottom: '1.5rem', display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          >
            ...
            <option value="suivi">Heures travaillées</option>
            <option value="photo">Photos</option>
            <option value="pdf">Documents PDF</option>
            <option value="screenshot">Captures écran</option>
          </select>
        </div>
                ...
                {items.map((item, idx) => (
                  <div key={idx} style={{ 
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
                      ...
                      {item.technicien && (
                        <span style={{ color: '#6b7280' }}>par {item.technicien}</span>
                      )}
